...
cc.initialize_cache('/home/huzheyuan/octo/jax_cache')
...
flags.DEFINE_string("video_dir", None, "Save directory for videos")
flags.DEFINE_string("goal", None, "Text goal for the robot to accomplish")
...
def stack_and_pad_obs(fn, horizon):
    ...
    def wrapped_fn(obs, *args, **kwargs):
        if isinstance(obs, list):
            POLICY_HISTORY.extend(obs)
        else:
            POLICY_HISTORY.append(obs)
        history = POLICY_HISTORY[-horizon:]
...
def load_checkpoint(weights_path):
    ...
    elif experiment_hparams['normalization'] == 'global_bounds':
        ac_min = np.array(action_metadata["global_min"])
        ac_max = np.array(action_metadata["global_max"])
        action_loc = 0.5 * (ac_max + ac_min)
        action_scale = 0.5 * (ac_max - ac_min)
    else:
        raise ValueError
    ...
    return policy_fn, action_denorm_fn, experiment_hparams, model, action_proprio_metadata['proprio_bimanual']


class OctoPolicy:
    def __init__(self, policy_fn, action_denorm_fn, exp_hparams, model, metadata):
        self.policy_fn = policy_fn
        self.action_denorm_fn = action_denorm_fn
        self.img_mapping = exp_hparams['img_mapping']
        self.cam_order = list(self.img_mapping.keys())
        self.model = model
        self.metadata = metadata

        text_goal = FLAGS.goal if FLAGS.goal else exp_hparams['text_goal']
        print(f"Using text goal: {text_goal}")
        self.goal = self.model.create_tasks(texts=[text_goal])

        self.hz = int(exp_hparams['hz'])
    ...
    def _normalize_proprio(self, proprio):
        mask = self.metadata.get("mask", np.ones_like(self.metadata['mean'], dtype=bool))
        return np.where(
                    mask,
                    (proprio-self.metadata['mean'])/(self.metadata['std']+1e-8),
                    proprio)
    ...
    def _infer_policy(self, observation):
        ...
        obs_dict['proprio_bimanual'] = observation['qpos'].astype(np.float32)
        # obs_dict['proprio_bimanual'] = self._normalize_proprio(obs_dict['proprio_bimanual'])


        obs_debug_img = [obs_dict[c] for c in self.cam_order]
        self.video.append(np.concatenate(obs_debug_img, axis=1))
    ...
    def _forward_chunked(self, observation):
        ...
        last_ac = self._last_ac if self._last_ac is not None else raw_ac
...
def main(_):
    ...
    for itr in range(FLAGS.num_rollouts):
        ...
        if FLAGS.video_dir:
            ...
            video_path = os.path.join(FLAGS.video_dir, f"rollout_{itr}.mp4")
            writer = imageio.get_writer(video_path, fps=policy.hz)
            for frame in policy.video:
