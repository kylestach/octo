...
from octo.model.components.vit_encoders import ResNet26FILM, ViTResnet
...
def get_config():
    ...
    return ConfigDict(
        dict(
            seed=42,
            num_steps=300000,
            save_dir='gs://multi-robot-bucket1/runs/',
            model=get_model_config("vit_s"),
            window_size=window_size,
            dataset_kwargs=get_dataset_config("multi", window_size, action_horizon=4),
            # skip_norm_keys=["proprio_primary"],
            optimizer=dict(
                learning_rate=dict(
                    name="rsqrt",
                    init_value=0.0,
...
def get_dataset_config(task_cond, window_size, action_horizon):
    ...
    return dict(
        oxe_kwargs=dict(
            ...
            load_camera_views=("primary", "secondary"), # we should have separate encoders for bridge and nav
            # load_proprio=False,
            load_depth=False,
        ),
...
def get_augmentation_config(task_cond, window_size, action_horizon):
    ...
    traj_transform_kwargs = dict(
        ...
        # max_proprio_dim=14, # bridge has 14, nav has 5
    ...
    frame_transform_kwargs = dict(
        ...
        image_augment_kwargs={
            "primary": image_augment_kwargs,
            "secondary": image_augment_kwargs,
        },
...
def get_model_config(transformer_size):
    ...
    return dict(
        ...
        readouts=dict(action=1),  # using 100 readout tokens for now because pooling strategy is pass on aloha head
