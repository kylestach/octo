...
flags.DEFINE_string("robot_config_path", "/scr/jhejna/robot-lightning/configs/iliad_franka_orca.yaml", "Path to robot config")
...
flags.DEFINE_integer("exec_horizon", 4, "Length of action sequence to execute")
...
class ILIADWrapper(gym.Wrapper):
    ...
    def step(self, action):
        # do the reverse of what was done
        gripper_action = action[-1]
        
        if gripper_action > 0.5:
            gripper_action = -1
        else:
            gripper_action = 0 # close halfway for coffee pod task.
...
def main(_):
    ...
    print(model.dataset_statistics["action"])
